...
def _read_cache() -> list | None:
    _jtrace(f"[TRACE] core.system.openrouter_catalog._read_cache: enter")
    try:
        ...
        _jtrace(f"[TRACE] core.system.openrouter_catalog._read_cache: except Exception")
        pass
    return None
...
def _stale_cache() -> list:
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.system.openrouter_catalog._stale_cache: except Exception")
        return []
...
def _write_cache(models: list) -> None:
    try:
        ...
        _jtrace(f"[TRACE] core.system.openrouter_catalog._write_cache: except {str(e)[:80]}")
        logger.warning(f"[openrouter_catalog] cache write failed: {e}")


def _fetch() -> list:
    _jtrace(f"[TRACE] core.system.openrouter_catalog._fetch: enter")
    import httpx
    headers = {}
    ...
    for m in data:
        ...
        def _per_mtok(v):
            try:
                ...
                _jtrace(f"[TRACE] core.system.openrouter_catalog._fetch._per_mtok: except ?")
                return None
        models.append({
            "id": m.get("id"),
...
def list_models(query: str | None = None, refresh: bool = False, limit: int = 50) -> list:
    ...
    _jtrace(f"[TRACE] core.system.openrouter_catalog.list_models: enter")
    ...
    if models is None:
        try:
            ...
            _jtrace(f"[TRACE] core.system.openrouter_catalog.list_models: except {str(e)[:80]}")
