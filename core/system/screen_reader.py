...
class JarvisScreenReader:
    ...
    async def read_screen(self) -> Dict[str, Any]:
        ...
        """
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader.read_screen: enter")
    ...
    async def _read_active_app(self, hwnd: int) -> Dict[str, Any]:
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_active_app: enter")
        if not hwnd:
            return {}
        try:
            ...
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_active_app: except {str(e)[:80]}")
            return {"error": str(e)}

    async def _read_window_stack(self) -> List[Dict[str, Any]]:
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_window_stack: enter")
        windows = []
            ...
            if win32gui.IsWindowVisible(hwnd):
                ...
                if title:
                    try:
                        ...
                        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_window_stack.enum_win: except Exception")
                        pass
        try:
            win32gui.EnumWindows(enum_win, None)
        except Exception:
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_window_stack: except Exception")
            pass
        return windows
    ...
    async def _read_accessibility_tree(self, hwnd: int) -> List[Dict[str, Any]]:
        ...
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_accessibility_tree: enter")
        children = []
        if not hwnd:
            return children
        ...
        def enum_child(child_hwnd, extra):
            try:
                ...
                _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_accessibility_tree.enum_child: except Exception")
                pass
        
        try:
            win32gui.EnumChildWindows(hwnd, enum_child, None)
        except Exception:
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_accessibility_tree: except Exception")
            pass
        return children

    async def _read_all_text(self, hwnd: int) -> List[str]:
        """Compile all structural texts from controls."""
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_all_text: enter")
    ...
    async def _read_web_content(self) -> Dict[str, Any]:
        ...
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_web_content: enter")
        active_hwnd = win32gui.GetForegroundWindow()
        if not active_hwnd:
        ...
        try:
            ...
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_web_content: except Exception")
        ...
        try:
            ...
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_web_content: except ClientConnectionError")
            return {"status": "cdp_not_available", "hint": "Launch browser with --remote-debugging-port=9222"}
        except Exception as e:
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._read_web_content: except {str(e)[:80]}")
            return {"status": "error", "message": str(e)}

    async def _synthesize_summary(self, graph: Dict[str, Any]) -> str:
        _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._synthesize_summary: enter")
...
"""
        try:
            ...
            _jtrace(f"[TRACE] core.system.screen_reader.JarvisScreenReader._synthesize_summary: except {str(e)[:80]}")
