...
def run_dag(user_task: str, dry_run: bool = False, task_id: str = None, user_id: str | None = None) -> dict:
    ...
    _jtrace(f"[TRACE] {__name__}.run_dag: enter task={user_task[:60]} task_id={task_id} dry_run={dry_run} recovered={is_recovered}")
    ...
    while orchestrator.current_index < len(orchestrator.execution_list):
        ...
        _jtrace(f"[TRACE] {__name__}.run_dag: submit subtask id={st['id']} agent={agent_role} desc={subtask_desc[:60]}")
        _future = _agent_executor.submit(agent.run, subtask_desc)
        try:
            result = _future.result(timeout=_AGENT_TIMEOUT_SEC)
            _jtrace(f"[TRACE] {__name__}.run_dag: subtask done id={st['id']} agent={agent_role} status={result.get('status') if isinstance(result, dict) else type(result).__name__}")
        except FuturesTimeoutError:
            _jtrace(f"[TRACE] {__name__}.run_dag: subtask TIMEOUT id={st['id']} agent={agent_role} after={_AGENT_TIMEOUT_SEC}s")
            result = {
                "status": "error",
