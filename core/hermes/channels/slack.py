...
def verify_slack_signature(request_body: str, timestamp: str, signature: str) -> bool:
    _jtrace(f"[TRACE] core.hermes.channels.slack.verify_slack_signature: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.hermes.channels.slack.verify_slack_signature: except ValueError")
        return False
...
def send_slack_message(channel: str, text: str, thread_ts: str = None) -> str:
    _jtrace(f"[TRACE] core.hermes.channels.slack.send_slack_message: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.hermes.channels.slack.send_slack_message: except {str(e)[:80]}")
        logger.error(f"Failed to send Slack message: {e}")
    return ""

def update_slack_message(channel: str, ts: str, text: str):
    _jtrace(f"[TRACE] core.hermes.channels.slack.update_slack_message: enter")
    token = os.environ.get("JARVIS_SLACK_BOT_TOKEN", "").strip()
    if not token:
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.hermes.channels.slack.update_slack_message: except {str(e)[:80]}")
        logger.error(f"Failed to update Slack message: {e}")

def execute_slack_prompt(channel: str, prompt: str, thread_ts: str):
    _jtrace(f"[TRACE] core.hermes.channels.slack.execute_slack_prompt: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.hermes.channels.slack.execute_slack_prompt: except {str(e)[:80]}")
...
def _parse_list(s: str) -> set[str]:
    _jtrace(f"[TRACE] core.hermes.channels.slack._parse_list: enter")
...
async def slack_events(
    ...
    _jtrace(f"[TRACE] core.hermes.channels.slack.slack_events: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] core.hermes.channels.slack.slack_events: except Exception")
