  ...
  return (
    <>
        ...
        <DCSection id="mission-control" title="Mission Control" subtitle="Three desktop directions · Cinematic Jarvis · Iron Man inspired">
          <DCArtboard id="v1-hud" label="A · HUD Cinematic" width={1440} height={920}>
            <MCVariantHUD width={1440} height={920}
              ...
              density={t.density} vizMode={t.vizMode} live={t.live} />
          </DCArtboard>
      ...
      <TweaksPanel title="Tweaks · Mission Control">
        ...
        <TweakSection label="Live Mode">
          <TweakToggle label="Connect to Jarvis backend"
            value={t.live}
            onChange={(v) => setTweak('live', v)} />
          <div style={{ fontSize: 10, lineHeight: 1.5, color: 'rgba(255,255,255,0.45)', marginTop: 6 }}>
            When ON: telemetry + WS data comes from the real Hermes server.
            Token is read from localStorage <code style={{ fontFamily: 'monospace' }}>hermesToken</code>.
          </div>
        </TweakSection>
        <TweakSection label="Notes">
          <div style={{ fontSize: 11, lineHeight: 1.55, color: 'rgba(255,255,255,0.55)' }}>
            ...
            DAG execute end-to-end (with auto-heal at the QA step). Toggle <b>Live Mode</b> above to
            overlay real Hermes data.
          </div>
        </TweakSection>
      </TweaksPanel>
    </>
