...
print(json.dumps(safe_clean(dry_run=True)))
"@
    ...
    foreach ($t in $dry.targets) {
        Write-Host ("    {0,-55} {1,6:F1} MB" -f ($t.path -replace "C:\\Users\\hasin", "~"), $t.mb) -ForegroundColor DarkGray
    }
    Write-Host ""
...
sys.path.insert(0, r'$PROJECT')
from tools.disk_cleanup import safe_clean
print(json.dumps(safe_clean(dry_run=False)))
"@
...
print(json.dumps(judgment_scan()))
"@
    ...
    foreach ($item in $items) {
        $i++
        $short = $item.path -replace "C:\\Users\\hasin", "~"
...
import json, sys
sys.path.insert(0, r'$PROJECT')
from tools.disk_cleanup import delete_judgment_item
print(json.dumps(delete_judgment_item(r'$Path')))
"@
