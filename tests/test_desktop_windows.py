...
# desktop_automation lazy-loads pygetwindow via _get_gw() (the old module-level
# `gw` attribute no longer exists) — patch the accessor instead.

@patch("tools.desktop_automation._get_gw")
def test_desktop_get_active_window_success(mock_get_gw):
    mock_gw = MagicMock()
...
@patch("tools.desktop_automation._get_gw")
def test_desktop_get_active_window_none(mock_get_gw):
    mock_gw = MagicMock()
    mock_get_gw.return_value = mock_gw
...
@patch("tools.desktop_automation._get_gw")
def test_desktop_focus_window_success_minimized(mock_get_gw):
    mock_gw = MagicMock()
    mock_get_gw.return_value = mock_gw
    mock_win = MagicMock()
...
@patch("tools.desktop_automation._get_gw")
def test_desktop_focus_window_success_not_minimized(mock_get_gw):
    mock_gw = MagicMock()
    mock_get_gw.return_value = mock_gw
    mock_win = MagicMock()
...
@patch("tools.desktop_automation._get_gw")
def test_desktop_focus_window_not_found(mock_get_gw):
    mock_gw = MagicMock()
    mock_get_gw.return_value = mock_gw
