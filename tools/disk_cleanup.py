...
def never_touch_check(path: str) -> bool:
    ...
    _jtrace(f"[TRACE] tools.disk_cleanup.never_touch_check: enter")
...
def _dir_size_bytes(path: str) -> int:
    ...
    _jtrace(f"[TRACE] tools.disk_cleanup._dir_size_bytes: enter")
    total = 0
    try:
        for dirpath, _dirs, files in os.walk(path):
            for f in files:
                try:
                    ...
                    _jtrace(f"[TRACE] tools.disk_cleanup._dir_size_bytes: except OSError")
                    pass
    except (PermissionError, OSError):
        _jtrace(f"[TRACE] tools.disk_cleanup._dir_size_bytes: except ?")
        pass
    return total
...
def _file_size_mb(path: str) -> float:
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._file_size_mb: except OSError")
        return 0.0
...
def scan() -> dict:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup.scan: enter")
    temp_bytes = (
        ...
        for fname in os.listdir(logs_dir):
            if fname.endswith(tuple(f".{i}" for i in range(1, 10))):
                try:
                    ...
                    _jtrace(f"[TRACE] tools.disk_cleanup.scan: except OSError")
                    pass
...
def _safe_remove(path: str, dry_run: bool) -> int:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup._safe_remove: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._safe_remove: except {str(e)[:80]}")
...
def _clean_dir_contents(directory: str, dry_run: bool) -> tuple[int, int]:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup._clean_dir_contents: enter")
    files_deleted = 0
    bytes_freed = 0
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._clean_dir_contents: except {str(e)[:80]}")
...
def safe_clean(dry_run: bool = True) -> dict:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup.safe_clean: enter")
    total_files = 0
    total_bytes = 0
    targets = []

    def _clean(path):
        _jtrace(f"[TRACE] tools.disk_cleanup.safe_clean._clean: enter")
...
def _days_since(path: str) -> int:
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._days_since: except OSError")
        return 0
...
def _has_recent_git_activity(directory: str, days: int = 30) -> bool:
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._has_recent_git_activity: except Exception")
...
def _ollama_suggest(description: str) -> str:
    ...
    _jtrace(f"[TRACE] tools.disk_cleanup._ollama_suggest: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup._ollama_suggest: except {str(e)[:80]}")
...
def judgment_scan() -> list[dict]:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup.judgment_scan: enter")
    candidates = []
        ...
        for dirpath, dirs, files in os.walk(root_dir):
            ...
            for fname in files:
                ...
                try:
                    ...
                    _jtrace(f"[TRACE] tools.disk_cleanup.judgment_scan: except OSError")
                    continue
...
def delete_judgment_item(path: str) -> dict:
    ...
    """
    _jtrace(f"[TRACE] tools.disk_cleanup.delete_judgment_item: enter")
    if never_touch_check(path):
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.disk_cleanup.delete_judgment_item: except {str(e)[:80]}")
