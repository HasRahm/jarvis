...
def _setup_tesseract():
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.hybrid_cursor._setup_tesseract: except ImportError")
        return None
...
def _layer1_graph_focus(window_hint: str, view_session=None) -> tuple:
    ...
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor._layer1_graph_focus: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.hybrid_cursor._layer1_graph_focus: except {str(exc)[:80]}")
...
def _layer2_mouse_braille(target: str, window_node: dict, view_session=None) -> tuple:
    ...
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor._layer2_mouse_braille: enter")
    try:
        import uiautomation as auto
    except ImportError:
        _jtrace(f"[TRACE] tools.hybrid_cursor._layer2_mouse_braille: except ImportError")
    ...
    def probe_grid(step: int, y_end: int) -> tuple:
        _jtrace(f"[TRACE] tools.hybrid_cursor._layer2_mouse_braille.probe_grid: enter")
        for py in range(y_start, y_end, step):
            for px in range(x_start, x_end, step):
                try:
                    ...
                    _jtrace(f"[TRACE] tools.hybrid_cursor._layer2_mouse_braille.probe_grid: except Exception")
                    continue
        return False, None
...
def _layer3_ocr(target: str, view_session=None, window_bounds=None) -> tuple:
    ...
    Returns:
        (success: bool, message: str)
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor._layer3_ocr: enter")
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.hybrid_cursor._layer3_ocr: except {str(exc)[:80]}")
...
def _check_text_on_screen(text: str, min_conf: int = 40) -> tuple:
    ...
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor._check_text_on_screen: enter")
    pytesseract = _setup_tesseract()
    if pytesseract is None:
    ...
    try:
        ...
        _jtrace(f"[TRACE] tools.hybrid_cursor._check_text_on_screen: except {str(exc)[:80]}")
...
def _verify_outcome(
    ...
    Returns:
        (success: bool, message: str)
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor._verify_outcome: enter")
...
def hybrid_locate_click(
    ...
    """
    _jtrace(f"[TRACE] tools.hybrid_cursor.hybrid_locate_click: enter")
    ...
    for attempt in range(max_retries + 1):
        ...
        if verify_text:
            try:
                ...
                _jtrace(f"[TRACE] tools.hybrid_cursor.hybrid_locate_click: except {str(_e)[:80]}")
