...
class BaseAgent(ABC):
    ...
    def _call_model(self, system_prompt: str, user_prompt: str) -> str:
        ...
        _jtrace(f"[TRACE] {__name__}.BaseAgent._call_model: enter role={self.role} model={self.model}")
        if os.environ.get("JARVIS_CI") == "true":
        ...
        _jtrace(f"[TRACE] {__name__}.BaseAgent._call_model: chain={[str(m)[:40] for m in models_to_try]}")
        for model_str in models_to_try:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    _jtrace(f"[TRACE] {__name__}.BaseAgent._call_model: attempt failed model={model_str} attempt={attempt+1} err={str(e)[:80]}")
    ...
    def _raw_call(self, provider: str, model: str, system_prompt: str, user_prompt: str) -> str:
        ...
        _jtrace(f"[TRACE] {__name__}.BaseAgent._raw_call: provider={provider} model={model} in_chars={len(system_prompt)+len(user_prompt)}")
        ...
        try:
            ...
            latency = time.time() - start_time
            _jtrace(f"[TRACE] {__name__}.BaseAgent._raw_call: ok model={model} latency={latency:.2f}s out_tokens={output_tokens}")
            record_api_telemetry(self.role, model, latency, input_tokens, output_tokens, status)
            ...
            _jtrace(f"[TRACE] {__name__}.BaseAgent._raw_call: EXCEPTION provider={provider} model={model} err={str(e)[:80]}")
            status = "error"
            latency = time.time() - start_time
